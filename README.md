...
- **Chrome 语音识别网络依赖**：Chrome 的 Web Speech API 走 Google 在线服务，国内网络不通时静默报 `network` 错误（原代码静默重启="没反应"）；解决：开启系统代理（ProxyEnable=1）+ 一键脚本以 `--proxy-server=http://127.0.0.1:10808` 强制 Chrome 走代理（独立 user-data-dir 不干扰日常浏览器）；Edge 的识别走微软服务、国内直连可达（备选）
...
- **UI 细节**：长文本/公式溢出修复（word-break）、对话容器改为 width calc + auto margin 居中（两侧对称）、favicon 404 抑制、TTS 后端重试 3 次缩为 2 次缩短失败等待
