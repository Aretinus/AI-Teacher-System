...
### P46（体验增强）. 引入外部设计 Skill + 高级化视觉轮
- **背景**：用户要求"引入页面设计 Skills（从 GitHub 下载）并优化页面，要高级一点"。
- **引入**：`01-Skills/web-design/`（github.com/xiaopu-ai/web-design，MIT）——DESIGN.md 两阶段工作流 + 75 个顶级产品设计系统拆解（Linear/Stripe/Notion/Apple…）+ 交互三档模式库 + 动效库 + 风格种子 + 设计令牌爬取脚本。
- **落地**：①产出 `04-Frontend/DESIGN.md`（清风蓝系统正式文档化，9 章节）；②高级化打磨——浅色换深藏青文字（#172033）+ Stripe 式蓝调多层投影（rgba(50,50,93)）+ 半透明边框 + 圆角收紧（20→16rpx）+ 标题负字距；深色换 Linear 式蓝调近黑分层（#101116/#17191f）+ 半透明白边框（rgba 白 5-8%）+ 近白文字；③基础设施——平滑滚动、prefers-reduced-motion 降级、focus-visible 焦点环。
- **经验**：顶级设计系统的"高级感"公式高度一致——低对比半透明边框造结构、多层带色相的投影造深度、标题负字距造精致、单一克制强调色。
- **状态**：已完成（2026-08-30，浅/深双主题浏览器验证）。

### P45. 同一对话中 AI"乱回答"：语音误识别 + 提示词跨课程串味（全局状态污染）
- **现象**：教《毛泽东诗词全编鉴赏》时，AI 突然聊起《矛盾论》/"共产主义相关的哲学文本"，把不相关课程的内容混进回答；对明显乱码的输入（"帧率真六""试吃"）硬猜硬答不确认。
- **根因（会话 20260830-238 证据）**：①用户走语音输入，ASR 误识别多（"诗词"→"狮子"、"诗词"→"试吃"、"正编"→"帧率真六"），模型对乱码只能硬猜；②提示词里的「用户当前状态」（knowledgePoints 含数学/物理/哲学全部课程）与「近期会话摘要」（含矛盾论等他课会话）是**用户级全局数据**——教诗词课时把其他学科的学习记录全塞进上下文，模型自然串课；③（已由 P44 修复的）"不得跳章"指令加剧了拉扯。
- **解决**：①课程模式与大类综合问答的提示词做**状态隔离**：knowledgePoints 与近期会话摘要只注入当前学科的条目（全科问答保持全局）；②语音链路打标：voice-call 提交问题时带 `voice: true`，一路透传到提示词，附指令"消息可能存在同音字/断句错误，先按同音字推测原意并向学生确认，再教学"；③三个 promptBuilder 构建器统一支持 voiceInput。
- **经验**："AI 乱回答"优先排查提示词里实际注入了什么——用户级全局状态（学习记录/会话摘要）在多课程场景是最常见的污染源；语音输入场景再叠一层 ASR 误识别，需要显式告知模型而非让它硬猜。
- **状态**：已修复（2026-08-30）。

### P44. 课程模式无法跳章：学生要求"直接教第七章"时模型卡在书名页并谎称"没蒸馏好"
- **现象**：会话 20260830-238（《毛泽东诗词全编鉴赏》）：学生多次要求"跳到第七章正编/直接教第一首诗词"，模型反复停留在第01课《书名页》，最后辩称"第七章的 117 个 md 还没整理到位/存在≠可教"——但实际上第07章的 117 课**早已蒸馏完成**。
- **根因**：两层。①课程上下文只给模型"当前课"的正文 + 章名列表，第07章的课文根本不在提示词里，模型无从教起；②提示词里有硬指令"**不得跳章**"，学生要求跳转时模型被指令困住，只能编造"未蒸馏"的借口。
- **解决**：①`loadDistilledCourseContext` 新增 `parseJumpTarget`：从学生消息解析"第X章/第X课"跳转意图（中文数字/阿拉伯数字均可，如"第七章"→7），命中则把 current 跳到该章第一课/对应课，并持久化进度（后续"下一课"从新位置继续）；②"跳过前置内容/直接进入"类消息自动跳到第一个非前置章（跳过书名页/版权页/目录等）；③提示词配合：有跳转时把"不得跳章"替换为"已按学生要求跳转，直接教该课"，`chapterList` 升级为带每章课数的导航（第07章_正编（117 课）…）。
- **经验**：模型"编借口"（内容没准备好/不能教）往往不是蒸馏缺失，而是**上下文没把内容给到 + 指令禁止了它做学生要求的事**——先检查提示词给了什么、禁了什么，再怀疑数据。
- **状态**：已修复（2026-08-30，端到端验证：直接从《贺新郎·别友》开始教学）。
