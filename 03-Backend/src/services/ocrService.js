...
// 是否可用 OCR 生成文字层（其余格式 epub/txt 等直接归入无需OCR）
function isOcrable(ext) {
  return ext === '.pdf' || ext === '.djvu';
}
...
async function scanRaw(subject) {
  ...
  function walk(dir, rel) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        const done = isOcrable(ext) && fs.existsSync(prod.main);
  ...
  const toProbe = files.filter((f) => isOcrable(f.ext) && (!status[f.file] || !status[f.file].kind));
