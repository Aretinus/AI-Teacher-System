...
function buildSystemPrompt({ subject, tutor, userState, history, message, style, voiceInput }) {
  ...
  return [
    ...
    `学生消息：${message}`,
    ...(voiceInput ? [
      '（本条消息来自语音识别：几乎没有标点、可能有同音字错误。请按以下步骤理解：①按语义自行断句并补回标点，再判断这句话的语气——疑问、陈述、肯定、感慨还是指令；②按语气回应：疑问就解答，陈述/肯定就点评与鼓励，指令就执行；③同音字按上下文推测原意（如「狮子」可能是「诗词」），推测不出再向学生确认。示例：「这本书里面有哪些诗词 是疑问 要给出例子」=「这本书里面有哪些诗词？」——疑问句，需列举。）',
    ] : []),
...
function buildCourseSystemPrompt({ subject, tutor, userState, history, message, courseCtx, style, voiceInput }) {
  ...
  return [
    ...
    `学生消息：${message}`,
    ...(voiceInput ? [
      '（本条消息来自语音识别：几乎没有标点、可能有同音字错误。请按以下步骤理解：①按语义自行断句并补回标点，再判断这句话的语气——疑问、陈述、肯定、感慨还是指令；②按语气回应：疑问就解答，陈述/肯定就点评与鼓励，指令就执行；③同音字按上下文推测原意（如「狮子」可能是「诗词」），推测不出再向学生确认。示例：「跳到第七章」= 指令——直接跳转开始教学。）',
    ] : []),
