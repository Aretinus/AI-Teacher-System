...
### 语音交互体验完善（2026-08-21 下午）
- **悬浮球**：语音通话最小化后对话页显示可拖动悬浮球——按住拖动（鼠标/触摸、贴边限位、避让 H5 导航栏）、轻点展开环形扇区菜单（SVG 弧段：挂断/进入，沿圆边分布，贴边自动换朝向，未来功能追加列表即可均分）；球体颜色随通话阶段变化（绿=聆听/蓝=思考/橙=回答）
- **只答最新一次**：AI 思考/生成期间麦克风保持开启；此时补充或更正的新问题会自动中止旧请求并覆盖（请求代际机制，旧回复回调全部作废）；Chrome 提前断句产生的半截提交同样被新问题覆盖
- **通话生命周期修复**：「取消通话」在无历史栈时兜底跳回首页（不再表现为刷新重启通话）；浏览器返回/手势离开语音页自动挂断；「最小化」保持通话并与对话页实时同步
- **显示细节**：语音页 AI 回复改用 md-render 渲染（Markdown/KaTeX 正确显示，深色主题适配）；对话页/语音页消息底部留白不再贴边

### 书库实时学科与交互优化（2026-08-21）
- **书库学科实时化**：新增 `GET /api/books/subjects`（已注册学科 + 实时扫描书库 raw/ocr/distilled 顶层目录并集，未注册文件夹自动补入，含中文映射）；设置页学科 chips 改用该接口且每次扫描时静默刷新——新放入书库的文件夹立即出现
- **未注册学科通用教学**：主页显示全部学科；对话路由对未注册学科不再报"学科不支持"，走通用教学模式（无技能/课程上下文，按学科名直接教学）
- **OCR 全格式 + 异步探查**：OCR 层扫描全部书籍格式（epub/mobi/txt 等归入「无需OCR」，数量不再缺失）；文字层探查改异步（修复同步阻塞后端导致全站接口挂起、页面数据"全部消失"，见 P29）
- **课程发现兜底**：courseService bookDirOf 同样兜底未注册学科——蒸馏新书后主页立即出现对应课程（文学已验证）
- **书籍大小显示**：sizeMB 精度 2 位小数；前端不足 1MB 显示 KB
- **消息时间**：对话页/语音页每条消息下显示时间（今天 HH:MM，跨天带日期）；主页最近学习与会话记录面板显示最近对话时间（lastAt）；打开旧会话自动滚到最新一条
- **语音通话交互重构**：AI 播报期间停止收录麦克风（回声问题根除，移除自动打断逻辑）；新增手动「打断」按钮（思考中可中止请求、播报中可停声音，均直接回到聆听）；思考中显示"正在思考···"动画提示；语音页消息带时间
- **双页文本实时同步**：进入语音通话自动载入当前会话已有文本；通话中的每句话实时出现在对话页；示例提示词按学科区分（数学/物理/经济学/文学/哲学/道家/中医）
...
| Literature（文学） | The Altruist in Politics 政坛上的利他主义者 | `Literature/英文小说/`（1 课） | 未注册（通用教学） |
| Philosophy（哲学） | 毛泽东选集 第1–4卷 | `Philosophy/共产主义/毛泽东/毛选/`（4 册各 1 课） | 未注册（通用教学） |

> 旧蒸馏课程（费恩曼物理学讲义、朗道、量子力学发展史、渐近分析、数学物理方法）已随清理从仓库删除；后续可用新公式管道重蒸。未注册学科的课程由书库目录实时发现，无需注册技能即可在主页选课学习。