...
- **语音输入理解（P45）**：语音提交的问题带 `voice` 标记，提示词告知模型"消息可能存在同音字/断句错误"——模型会先断句、判断语气（疑问/陈述/肯定/指令）、按同音字推测原意，推测不出再向学生确认
- **课程模式跳转（P44）**：对话中直接说"跳到第X章 / 第X课 / 跳过书名页直接开始"即可跳转（支持中文数字），进度持久化，后续"下一课"从新位置继续
